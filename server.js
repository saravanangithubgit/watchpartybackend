...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', (data) => {
    ...
    roomUsers[roomId] = roomUsers[roomId].filter(
      (user) => user.id !== socket.id && user.name !== userName
    );
  ...
  socket.on('disconnect', () => {
    ...
    if (hostTracker[socket.id]) {
      ...
      delete roomMemory[roomId];
      delete roomUsers[roomId];
    } else if (joinedRoomId && roomUsers[joinedRoomId]) {
      roomUsers[joinedRoomId] = roomUsers[joinedRoomId].filter((user) => user.id !== socket.id);
      io.to(joinedRoomId).emit('update-users', roomUsers[joinedRoomId]);

      if (roomUsers[joinedRoomId].length === 0) {
        delete roomUsers[joinedRoomId];
      }
    } else {
      for (const currentRoomId in roomUsers) {
        const nextUsers = roomUsers[currentRoomId].filter((user) => user.id !== socket.id);
        if (nextUsers.length !== roomUsers[currentRoomId].length) {
          roomUsers[currentRoomId] = nextUsers;
          io.to(currentRoomId).emit('update-users', roomUsers[currentRoomId]);
